...
#        overall_dos /= 2.0
    ...
    pylab.plot(energy-ef, overall_dos, lw ='2', color=color[count]) #ls=linetypes[count] 

    max_dos = max(max_dos,numpy.amax(overall_dos))
    min_energy = max(min_energy,min(energy)-ef)
    max_energy = min(max_energy,max(energy)-ef)

    print ef 
    pylab.axvline(x=0, ymin=0, ymax=1, ls='--', color=color[count], lw='2')
    count += 1

pylab.xlabel(r'$\epsilon - \epsilon_{f} \rm{(eV)}$', fontsize=32)

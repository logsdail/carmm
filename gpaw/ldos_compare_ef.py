...
# Interpolated filled plots STRONG COLOURS
    ...
    if len(ef[ifile]) > 1:
        ...
        for ielement in range(len(ef[ifile])):
            ...
            pylab.fill_between(energy[ifile][0]-ef[ifile][ielement], current, previous, facecolor=colours[colour_counter], lw=0.0, interpolate=True, label=elements[ifile][ielement])
...
#        pylab.fill_between(energy[ifile][0]-ef[ifile], dos[ifile][0], y2=0, where=None, facecolor='orange', lw=0.0, interpolate=True, label=elements[ifile][0])
    ...
    pylab.axvline(x=0, ymin=0, ymax=1, ls='--', color='black', lw='1.0')

    if ifile == 0:
        pylab.xlabel(r'$\epsilon$ - $\epsilon_{f}$')
        pylab.xticks(range(-20,5,5))
    else:

...
Filename: CPentBi.cpp	Create the pentagonal bipyramid true dec structure
Description:Create the pentagonal bipyramid true dec structure
...
void CPentBi::generate_vertices(int Shell_num)
{
...
void CPentBi::generate_edges( int shell_num)
{
	edge_atoms.clear();
	vec3d new_point;
	double atom_spacing;
 ...
 // int index[] = {0,1,2, 0,3,4, 2,3,10 ,0,3,11, 4,5,8, 5,6,9, 6,7,10, 4,7,11};
	...
	{ 
		...
		{  
			...
			point_on_3D_line(vertices[6], vertices[i], new_point, atom_spacing * j);
			edge_atoms.push_back(new_point);
		}
	}
}

void CPentBi::generate_face111( int shell_num)
{
	single_face.clear();
  ...
  //make 2 edges for single face
	...
	for(int j = 1; j < shell_num; j++)
	{    
		point_on_3D_line(vertices[6], vertices[2], new_point, atom_spacing * j);
		edge_atoms.push_back(new_point);
	}
...
void CPentBi::generate_face100( int shell_num)
{
}

void CPentBi::generateCoordinates(const int &shell_num, std::string element)
{
	vec3d temp;
	if(shell_num < 0)
	{
...
void CPentBi::generateSingleShell(const int &shell_num, std::string element)
{
	vec3d temp;
	switch(shell_num)
	{
...
void CPentBi::combine_arrays(std::string element)
{
	vec3d temp;

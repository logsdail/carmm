...
void cXYZReadWrite::RandomiseClusterElements(std::string elementA, std::string elementB, int seed)
{
	srand(seed);
	int elementACount, elementBCount;
        float elementAProbf, elementBProbf;
        int elementAProbi, elementBProbi;
	elementACount = elementBCount = 0;
        elementAProbf = 0.5;			// This could be softcoded at some point
        elementBProbf = 1 - elementAProbf;
        elementAProbi = (int) round(elementAProbf*100);
        elementBProbi = (int) round(elementBProbf*100);

        std::cout << "Probabilities : " << elementA << " = " << elementAProbi << "%, " << elementB << " = " << elementBProbi << "%" <<  std::endl;
	for(int i = 0; i < m_ClusterInfo.atom.size(); i++)
	{
		int randomNumber = rand() % 100;
		if(randomNumber < elementAProbi)
		{
			m_ClusterInfo.atom[i].atomType = elementA;
			elementACount++;
		}
		else
		{
			m_ClusterInfo.atom[i].atomType = elementB;
			elementBCount++;
		}
	}
	std::cout << "Results : " << elementA << " = " << elementACount << ", " << elementB << " = " << elementBCount << std::endl;
}
...
void cXYZReadWrite::WriteFirst(std::fstream *pFile)
{
	if(pFile->is_open())
	{                
		*pFile << "0.000" << endl;

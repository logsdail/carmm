...
void CMinimiser::minimise()
{
	...
	// this->PrintMinimiserOptions(); // Surpressed to control outputs
	...
	for(;;)//an infinite for loop so must have break case
	{
		...
		f = this->fx_dx(ptrX,ptrG);
                ...
                // Added by Andrew as  to find out gradient problems with cutoff for testing
		...
		{
			// std::cout << "fxdx requested" << std::endl; // Surpressed to control outputs
			continue;
		}
		//if it returns false we should check for convergence
		if(is_Converged(ptrX,ptrG))
		{
			// std::cout << "Convergence obtained" << std::endl; // Surpressed to control outputs
			// std::cout << "gnorm" << m_pMinimizer->euclidean_norm(&(*(g.begin()))) << std::endl; // Surpressed to control outputs
			// std::cout << "nfunc: " << m_pMinimizer->nfun() << std::endl; // Surpressed to control outputs
			// std::cout << "iter: " << m_pMinimizer->iter() << std::endl; // Surpressed to control outputs
			break;
		}
